...
onMounted(() => {
  ...
  document.addEventListener("mouseup", (e) => {
    const editNameInput = document.getElementById(`editNameInput-${link.value.id}`);
    if (
      editNameInput
  ...
  document.addEventListener("mouseup", (e) => {
    const editLinkInput = document.getElementById(`editLinkInput-${link.value.id}`);
    if (
      editLinkInput
...
function changeInput(str, linkIdNameString) {
...
function editName(selectedId, selectedStr) {
...
function editLink(selectedId, selectedStr) {
  if (userStore.isMobile) {
    userStore.updatedLinkId = selectedId;
    return false;
  }
...
async function updateLinkImage() {
  try {
...
<template>
  <div :id="`LinkBox${link.id}`" class="w-full bg-white shadow rounded-md">
